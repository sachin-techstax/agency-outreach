...
> Production browser access uses two surfaces: `nuntago-demo.ergorum.com` is a read-only fictional showcase backed only by `/api/public/*`; `nuntago.ergorum.com` is the live operator workspace and should remain behind Cloudflare Access. The origin remains localhost-bound behind Cloudflare Tunnel.
